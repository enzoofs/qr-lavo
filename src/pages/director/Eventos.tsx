  ...
  return (
    <div className="min-h-full bg-lavo-paper p-6 max-w-md mx-auto">
      ...
      <div className="flex justify-between items-center mb-3">
        <h2 className="font-display text-xs tracking-wide text-lavo-ink">ENSAIOS</h2>
        {events.length > 0 && (
          <button onClick={toggleAll} className="text-xs font-bold text-lavo-aqua">
      ...
      {loading ? (
        ...
        <p className="text-lavo-muted text-sm">Nenhum ensaio ainda. Cria o primeiro acima.</p>
      ) : (
              ...
              <li key={ev.id} className="flex items-stretch gap-2">
                ...
                >
                  <input
                    ...
                    className="w-5 h-5 accent-lavo-blue"
                  />
                </label>
                <Link
                  to={`/director/eventos/${ev.id}`}
                  className={`flex-1 block p-4 rounded-md border-2 bg-white ${
                    isSelected ? 'border-lavo-blue' : 'border-lavo-ink'
                  }`}
                >
                  <div className="flex justify-between items-start gap-2">
                    <div>
                      <p className="font-bold text-lavo-ink">{ev.name}</p>
      ...
      {selected.size > 0 && (
        <div className="fixed bottom-0 left-0 right-0 p-4 bg-lavo-paper border-t-2 border-lavo-ink">
