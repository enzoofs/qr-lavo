...
function PinIcon() {
  return (
    <svg width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="#0b2340" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
...
function CheckIcon() {
  return (
    <svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="#0b2340" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
...
export default function Checkin() {
  ...
  return (
    <div className="min-h-full p-6 flex flex-col justify-center max-w-md mx-auto bg-lavo-paper">
      {status.kind === 'locating' && (
        <div className="text-center">
          ...
          <p className="text-sm text-lavo-muted mt-1">Aguarde uns segundos.</p>
        </div>
      )}
      ...
      {status.kind === 'submitting' && (
        <div className="text-center">
          <div className="animate-pulse mb-4 flex justify-center">
            <div className="w-10 h-10 rounded-full border-4 border-lavo-ink border-t-transparent animate-spin" />
          </div>
      ...
      {status.kind === 'success' && (
        <div className="text-center">
          <div className="flex justify-center mb-4">
            <div className="w-20 h-20 rounded-full bg-lavo-green border-2 border-lavo-ink flex items-center justify-center shadow-hard-sm">
              <CheckIcon />
            </div>
          </div>
          <h1 className="text-2xl font-extrabold mb-2">Presença registrada!</h1>
          {status.result.event_name && <p className="text-lavo-ink">{status.result.event_name}</p>}
          {status.result.distance_meters !== undefined && (
            <p className="text-xs text-lavo-muted mt-1">{status.result.distance_meters}m do local</p>
          )}
      ...
      {status.kind === 'error' && (
        <div className="text-center">
          <div className="flex justify-center mb-4">
            <div className="w-20 h-20 rounded-full bg-lavo-cyan border-2 border-lavo-ink flex items-center justify-center shadow-hard-sm">
              <WarnIcon />
            </div>
          </div>
          <h1 className="text-2xl font-extrabold mb-2">{status.message}</h1>
          {status.subline && <p className="text-lavo-ink">{status.subline}</p>}
