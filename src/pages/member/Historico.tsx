  ...
  return (
    <div className="min-h-full bg-lavo-paper p-6 max-w-md mx-auto">
      ...
      {loading ? (
        ...
        <p className="text-sm text-lavo-muted text-center py-6">Nenhum ensaio ainda.</p>
      ) : (
        <ul className="space-y-2">
          {rows.map((row) => {
            ...
            return (
              <li
                key={row.event.id}
                className="flex justify-between items-start gap-2 p-3 rounded-md bg-white border-2 border-lavo-ink"
              >
                <div className="min-w-0">
                  <p className="font-bold text-lavo-ink truncate">{row.event.name}</p>
                  <p className="text-xs text-lavo-muted mt-0.5">
