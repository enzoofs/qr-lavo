...
export function MemberImport({ onImportComplete, onCancel }: Props) {
  ...
  return (
    <div className="bg-white border-2 border-lavo-ink rounded-md p-4 mb-4">
      <div className="flex justify-between items-center mb-3">
        <h2 className="font-display text-[11px] tracking-wide text-lavo-ink">IMPORTAR PLANILHA</h2>
        <button onClick={onCancel} className="text-xs font-bold text-lavo-muted">
          FECHAR
        </button>
      </div>
        ...
        <div className="mt-3 text-xs">
          <p className="text-lavo-ink">
            ...
            {valid.length !== dedupedValid.length && (
              <>
                {' · '}
                <span className="text-lavo-muted">
          ...
          {invalid.length > 0 && (
            <ul className="mt-2 space-y-0.5">
              {invalid.slice(0, 5).map((r, i) => (
                <li key={i} className="text-lavo-red">
                  Linha: <span className="font-mono">{r.full_name || '(vazio)'}, {r.email || '(vazio)'}</span> — {r.error}
                </li>
              ))}
              {invalid.length > 5 && (
                <li className="text-lavo-muted">… e mais {invalid.length - 5}</li>
              )}
            </ul>
          )}
        </div>
      ...
      {error && <p className="text-sm font-semibold text-lavo-red mt-3">{error}</p>}

      {result && (
        <div className="mt-3 text-sm bg-lavo-green/20 border-2 border-lavo-ink text-lavo-ink rounded-md p-3">
          <p>
