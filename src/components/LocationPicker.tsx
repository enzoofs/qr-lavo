  ...
  return (
    <div className="space-y-2">
      <div className="relative">
        ...
        />
        {searching && (
          <span className="absolute right-3 top-3 text-xs text-lavo-muted">buscando…</span>
        )}
        {showResults && results.length > 0 && (
          <ul className="absolute z-[1000] mt-1 w-full bg-white border-2 border-lavo-ink rounded-md shadow-hard-sm max-h-60 overflow-y-auto">
            {results.map((r, i) => (
              <li key={i}>
                <button
                  type="button"
                  onClick={() => pickFromSearch(r)}
                  className="w-full text-left px-3 py-2 text-sm hover:bg-lavo-paper border-b border-lavo-ink/10 last:border-b-0"
                >
                  {r.display_name}
                </button>
              </li>
        ...
        {showResults && results.length === 0 && !searching && query.trim().length >= 3 && (
          <div className="absolute z-[1000] mt-1 w-full bg-white border-2 border-lavo-ink rounded-md shadow-hard-sm px-3 py-2 text-sm text-lavo-muted">
            Nenhum resultado.
          </div>
        )}
      </div>

      <div className="h-[300px] rounded-md overflow-hidden border-2 border-lavo-ink">
        <MapContainer
          ...
          {hasPin && (
            <>
              ...
              <Circle
                ...
                radius={radius}
                pathOptions={{ color: '#2f6fed', fillColor: '#2f6fed', fillOpacity: 0.15 }}
              />
      ...
      <div className="flex flex-col gap-1">
        <button
          ...
          onClick={pickFromGPS}
          disabled={gpsBusy}
          className="w-full py-2.5 rounded-md border-2 border-lavo-ink bg-white hover:bg-lavo-paper text-sm font-bold disabled:opacity-50 flex items-center justify-center gap-2"
        >
          <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="#0b2340" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
            <path d="M12 21s-7-6.5-7-11a7 7 0 0 1 14 0c0 4.5-7 11-7 11z" />
            <circle cx="12" cy="10" r="2.5" />
          </svg>
          {gpsBusy ? 'Capturando…' : 'Usar minha localização atual'}
        </button>
        {gpsError && <p className="text-xs font-semibold text-lavo-red">{gpsError}</p>}
        {hasPin && (
          <p className="text-xs text-lavo-muted">
